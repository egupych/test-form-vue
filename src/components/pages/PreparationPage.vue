...
const startPreview = () => {
  ...
  if (!selectedFormatId.value && availableFormats.value.length > 0) {
    selectedFormatId.value =
      availableFormats.value.find((f) => f.name === 'A4')?.name ||
      availableFormats.value[0].name;
  }
};
...
const saveCalibration = () => {
  ...
  const calculatedDevicePxPerMm =
    (calibrationWidthPx.value * window.devicePixelRatio) / creditCardWidthMm;
  userDevicePxPerMm.value = calculatedDevicePxPerMm;
  localStorage.setItem(
    'screenDevicePxPerMm',
    calculatedDevicePxPerMm.toString()
  );
  startPreview();
};
</script>
...
<template>
  <main class="py-10 md:py-25">
    <div class="max-w-6xl mx-auto px-4 md:px-0">
        ...
        >
          <div v-if="activeTab === tab.id">
            <div
              ...
              class="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-5 gap-6"
            >
              <a
              ...
              >
                <div
                  class="bg-light-gray w-full rounded-lg flex items-center justify-center p-4 mb-3 aspect-[3/4]"
                >
                  <svg
                    class="w-full h-full text-gray"
                    viewBox="0 0 100 133"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M20 1H80L99 20V113L80 132H20L1 113V20L20 1Z"
                      stroke="currentColor"
                      stroke-width="2"
                    />
                    <path
                ...
                <div class="text-body-panda text-panda-black mb-4">
                  {{ item.name }}
                </div>
          ...
          <div class="size-check-section">
            ...
            <div class="bg-white p-4 flex flex-col gap-4 h-full">
              <p class="text-body-panda text-dark-gray">
                Для точной работы потребуется быстрая калибровка экрана по
                банковской карте, карты имеют единый размер по стандарту ISO
                7810.
              </p>
              <BaseButton
          ...
          <div>
            ...
            <div class="bg-white p-4">
              <div
                ...
                :class="{
                  'border-b border-light-gray': index < dinFormats.length - 1,
                }"
              >
                <div class="text-body-panda text-panda-black">
                  <span class="font-bold">{{ format.name }}</span>
                </div>
                <div class="font-mono text-dark-gray">
                  {{ format.dimensions }}
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <div class="bg-white p-4">
              <div
                ...
                class="flex justify-between items-center py-3"
                :class="{
                  'border-b border-light-gray': index < cardFormats.length - 1,
                }"
              >
                <div class="text-body-panda text-panda-black font-bold">
                  {{ format.name }}
                </div>
                <div class="font-mono text-dark-gray">
                  {{ format.dimensions }}
        ...
        <div class="space-y-10">
          <div>
            ...
            <p class="text-dark-gray text-body-panda max-w-4xl">
              Макет желательно присылать в PDF. Если конвертации в PDF нет,
              допускается присылать макет в программах, где он был разработан
              (AI, CDR, PSD). Если макет был разработан в онлайн-редакторах
              (Figma, Canva), рекомендуется прикладывать ссылку для возможной
              корректировки.
            </p>
          </div>
          <div>
            ...
            <p class="text-dark-gray text-body-panda max-w-4xl">
              Необходимо во всех дизайн-макетах оставлять отступы с фоном по
              периметру по 3 мм для обреза. Например, если вы делаете визитку
              размером 90x50 мм, то размер макета с вылетами должен быть 96x56
              мм. Отступ содержимого (текста, изображений) от края реза должен
              составлять от 5 мм.
            </p>
          </div>
          <div>
            ...
            <p class="text-dark-gray text-body-panda max-w-4xl">
              Желательно отправлять файлы на печать в цветовой модели CMYK. Если
              вы работаете в программе, где нет цветовой модели CMYK, то
              старайтесь использовать цвета из палитры PANTONE. В таком случае,
              при работе в цветовой модели RGB (Figma, Canva), при использовании
              цветов PANTONE вы будете видеть макет почти таким же, каким он
              получится после печати.
            </p>
          </div>
          <div>
            <h3 class="font-semibold text-panda-black text-h5-panda mb-3">
          ...
          <div>
            ...
            <div class="space-y-3 text-dark-gray text-body-panda max-w-4xl">
              <p>
                Вектор — лучший выбор. Для четкой печати желательно использовать
                логотипы и иконки в векторном формате (SVG, AI, EPS, CDR).
                Векторные изображения печатаются без искажений и потери
                качества.
              </p>
              <p>
                1 мм при 300 DPI = 12 пикселей. Если вы делаете дизайн в
                программе, которая выдает размер только в пикселях (Figma,
                Canva), то нужно учитывать, что для 1 мм физического размера
                (при качественной печати 300 DPI) необходимо 12 пикселей.
              </p>
            </div>
          </div>
          <div>
            ...
            <div class="space-y-3 text-dark-gray text-body-panda max-w-4xl">
              ...
              <p>
                В макете не должно быть лишних элементов, чтобы они случайно не
                оказались на печати.
              </p>
            </div>
          </div>
      ...
      <div
        class="bg-light-gray rounded-2xl p-8 flex flex-col items-center justify-center text-center h-full mt-10 gap-page"
      >
        <img
              ...
              >
                ...
                >
                  ...
                  <svg
                    ...
                    :class="{ 'is-open': isFormatDropdownOpen }"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 20 20"
                  >
