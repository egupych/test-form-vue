...
const fetchWeather = async () => {
  try {
    const response = await fetch(
      'https://api.open-meteo.com/v1/forecast?latitude=51.18&longitude=71.45&current_weather=true&daily=weathercode,temperature_2m_max&hourly=temperature_2m,weathercode&timezone=Asia/Almaty'
    );
...
const isActivePeriod = (dayIndex, periodName) => {
  return (
    activePeriod.value.dayIndex === dayIndex &&
    activePeriod.value.periodName === periodName
  );
};
...
const processedForecast = computed(() => {
  if (!weather.value || !weather.value.daily || !weather.value.hourly)
    return null;
  ...
  return daily.time.slice(0, 5).map((date, index) => {
    ...
    if (index === 0) {
      ...
      dayText =
        index === 1
          ? 'Завтра'
          : forecastDate.toLocaleDateString('ru-RU', { weekday: 'short' });
    ...
    const hourlyTemps = hourly.temperature_2m.slice(
      startIndex,
      startIndex + 24
    );
    ...
    const details = [
      ...
      const avgTemp =
        tempsInPeriod.length > 0
          ? Math.round(
              tempsInPeriod.reduce((a, b) => a + b, 0) / tempsInPeriod.length
            )
          : 0;
      ...
      const hourlyDetails = timesInPeriod.map((t, i) => {
        ...
        return {
          time: new Date(t).toLocaleTimeString('ru-RU', {
            hour: '2-digit',
            minute: '2-digit',
          }),
...
<template>
  <div
    class="weather-widget dropdown"
    @mouseenter="showForecast = true"
            ...
            <div v-if="expandedIndex === dayIndex" class="detailed-forecast">
              <div class="time-periods-grid">
                <div
                  ...
                  class="time-period"
                  :class="{
                    'active-period': isActivePeriod(dayIndex, period.name),
                  }"
...
.expand-hourly-enter-active,
.expand-hourly-leave-active {
  transition: all 0.3s ease-out;
}
.expand-hourly-enter-from,
.expand-hourly-leave-to {
  opacity: 0;
  max-height: 0;
  padding: 0 0.5rem;
  margin-top: 0;
}
.expand-hourly-enter-to,
.expand-hourly-leave-from {
  max-height: 18.75rem;
}
.detailed-forecast,
.hourly-forecast {
